...
•	操作系统分内核与外壳（我们可以把外壳理解成围绕着内核的应用程序，而内核就是能操作硬件的程序）。
 ```
 ![](https://alibeibei.oss-cn-shanghai.aliyuncs.com/images/linux/sys.png
)
###### 1.2 操作系统简单分类
```
...
```
![](https://alibeibei.oss-cn-shanghai.aliyuncs.com/images/linux/diannao.png
)
```
3.	Linux: Linux是一套免费使用和自由传播的类Unix操作系统.Linux存在着许多不同的Linux版本，但它们都使用了 Linux内核 。Linux可安装在各种计算机硬件设备中，比如手机、平板电脑、路由器、视频游戏控制台、台式计算机、大型机和超级计算机。严格来讲，Linux这个词本身只表示Linux内核，但实际上人们已经习惯了用Linux来形容整个基于Linux内核，并且使用GNU 工程各种工具和数据库的操作系统。
 ```
 ![](https://alibeibei.oss-cn-shanghai.aliyuncs.com/images/linux/qie.png
)
### 二 初探Linux
###### 2.1 Linux简介
```
...
```
![](https://alibeibei.oss-cn-shanghai.aliyuncs.com/images/linux/linux.png
)
###### 2.3 Linux的分类
```
Linux根据原生程度，分为两种：
...
```
![](https://alibeibei.oss-cn-shanghai.aliyuncs.com/images/linux/linux-sys.png
)
### 三 虚拟机安装CentOS7
###### 3.1 VMware安装CentOS7
```
...
```
![](https://alibeibei.oss-cn-shanghai.aliyuncs.com/images/linux/linux-file.png
)
```
常见目录说明：
...
下面给出了一些比较常用的命令。推荐一个Linux命令快查网站，非常不错，大家如果遗忘某些命令或者对某些命令不理解都可以在这里得到解决。
...
o	同上，但忽略大小写: find /home -iname "*.txt"
...
示例：在随意某个目录下ls -l
```
![](https://alibeibei.oss-cn-shanghai.aliyuncs.com/images/linux/asas.png)
```
 
第一列的内容的信息解释如下：
```
![](https://alibeibei.oss-cn-shanghai.aliyuncs.com/images/linux/b1.png
)
```
...
chmod u=rwx,g=rw,o=r aaa.txt
 ```
 ![](https://alibeibei.oss-cn-shanghai.aliyuncs.com/images/linux/chmod.png
)
 ```
上述示例还可以使用数字表示：
chmod 764 aaa.txt
